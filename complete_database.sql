-- =====================================================
-- HUAYUE PLASTICS INDUSTRY - COMPLETE DATABASE QUERIES
-- Database: neondb (PostgreSQL on Neon platform)
-- Generated: July 4, 2025
-- =====================================================

-- =====================================================
-- 1. DATABASE CREATION (Already exists on Neon)
-- =====================================================
-- Database: neondb
...
-- NEWS TABLE (News and announcements)
...
-- ADMIN_USERS TABLE (Legacy admin storage)
...
-- =====================================================
-- 4. INSERT ACTUAL DATA (Current system data)
-- =====================================================

-- Insert actual admin user
INSERT INTO admins (id, email, password_hash, name, created_at) VALUES 
(3, 'admin@huayueplasticsindustry.com', '$2b$10$hashed_password_here', 'System Administrator', '2025-07-04 19:40:27.82469')
ON CONFLICT (id) DO NOTHING;

-- Insert actual contact messages
...
-- Insert actual products (HUAYUE and EIDER)
...
-- =====================================================
-- 5. COMMON QUERY OPERATIONS
-- =====================================================

-- Get all admin users
SELECT id, email, name, created_at,
CASE WHEN otp_code IS NOT NULL THEN 'OTP Active' ELSE 'No OTP' END as otp_status
FROM admins;
...
-- Get recent news
SELECT id, title, summary, publish_date, author 
FROM news 
...
WHERE (name ILIKE '%pipe%' OR description ILIKE '%pipe%') 
AND is_active = true;
...
UNION ALL SELECT 
    'news' as table_name, COUNT(*) as count FROM news
UNION ALL SELECT 
    'contact_messages' as table_name, COUNT(*) as count FROM contact_messages;
...
-- Add new product
INSERT INTO products (name, description, category, image_url, specifications, certifications) 
VALUES (?, ?, ?, ?, ?, ?);
...
UPDATE admins SET otp_code = NULL, otp_expiry = NULL 
...
-- Archive old news (older than 2 years)
UPDATE news SET is_active = false WHERE publish_date < NOW() - INTERVAL '2 years';

-- =====================================================
-- 8. CURRENT SYSTEM CREDENTIALS
-- =====================================================
-- Admin Login:
-- Email: admin@huayueplasticsindustry.com
-- Password: Admin4321!
-- Database: neondb (PostgreSQL)
-- Tables: 7 tables with data
-- Products: 12 items (6 HUAYUE + 6 EIDER)
-- Messages: 4 contact submissions
-- News: 1 article
-- =====================================================